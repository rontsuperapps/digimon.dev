/* ===================================================================
   MONITORING PROJECT — SERVICE WORKER (PWA)
   ===================================================================
   Tanggung jawab file ini HANYA app-shell (HTML/CSS/JS statis + font
   dari CDN), supaya halaman tetap bisa dibuka walau offline.

   Data dashboard (getMonitoringProjectData, getMonitoringGoLiveData)
   ...
   (semua panggilan fn berbeda akan menimpa entry cache yang sama kalau
...
self.addEventListener('fetch', (event) => {
  ...
  if (isSameOrigin) {
    ...
    // Aset CDN (Google Fonts, dll): stale-while-revalidate, supaya
    // halaman tetap bisa dirender walau offline. TIDAK berlaku untuk
    // GAS_EXEC_URL karena itu selalu POST (sudah ditangkap oleh guard
    // method !== 'GET' di atas).
    event.respondWith(
